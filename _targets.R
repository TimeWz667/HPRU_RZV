...
tar_option_set(
  packages = c("tibble", "tidyverse", "tidybayes", "ggtext", "rstan", "readxl", "glue")
)
...
list(
  ...
  ## Parameters
  tar_target(pars_bg, load_inputs_background(year0 = 2024, n_sims = n_sims)),
  ...
  tar_target(f_zvl, here::here("pars", "pars_ve_zvl_rw.rdata"), format = "file"),
  tar_target(f_rzv, here::here("pars", "pars_ve_rzv_" + glue::as_glue(vtype) + ".rdata"), pattern = map(vtype), format = "file"),
  tar_target(f_offset, here::here("pars", "fitted_ve_offset.rdata"), format = "file"),

  tar_target(pars_base, load_inputs(pars_ce, vtype, f_ve_zvl = f_zvl, f_ve_rzv = f_rzv, f_ve_offset = f_offset), pattern = cross(map(vtype, f_rzv), pars_ce)),
