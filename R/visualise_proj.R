...
vis_proj <- function(stats_proj) {
  ...
  tags <- c(
    "Null" = "No vaccine for HZ",
    "Stay" = "Keep ZVL, no eligibility change",
    "ToRZV" = "RZV, no eligibility change",
    "Sch65" = "RZV, lower age to 65 in 2023",
    "Sch" = "RZV, lower age to 60 in 2028",
    "Sch1d85" = "Add one-dose RZV to age 85",
    "Sch1d95" = "Add one-dose RZV to age 95",
    "Sch2d85" = "Add two-dose RZV to age 85",
    "Sch2d95" = "Add two-dose RZV to age 95"
  )
  ...
  gs$g_inc_all <- stats_proj$stats_proj_all %>% 
    ...
    filter(Scenario %in% c("Stay", "ToRZV", "Sch65", "Sch", "Sch1d95", "Sch2d95")) %>% 
    ...
    scale_y_continuous("Herpes zoster incidence per 100,000", labels = scales::number_format(scale = 1e5)) +
  ...
  gs$g_inc_bind <- ggpubr::ggarrange(
    gs$g_inc_all + labs(title = "**A**") + theme(legend.position = "None", plot.title = ggtext::element_markdown()),
    gs$g_inc_68 + labs(title = "**B**") + theme(legend.position = "right", plot.title = ggtext::element_markdown()),
